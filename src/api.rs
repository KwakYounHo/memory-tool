...
impl From<SearchHit> for HitDto {
    fn from(h: SearchHit) -> Self {
        Self {
            id: h.id, source: h.source, text: h.text,
            project: h.project, machine: h.machine,
            scope: h.scope, kind: h.kind, distance: h.distance,
        }
    }
}
...
pub async fn search_handler(
    ...
    let scopes: Option<Vec<Scope>> = req.filter.scope.as_ref().map(|ss| {
        ss.iter().filter_map(|s| match s.as_str() {
    ...
    let kinds: Option<Vec<Kind>> = req.filter.kind.as_ref().map(|ks| {
        ks.iter().filter_map(|k| match k.as_str() {
            "rule" => Some(Kind::Rule),
...
pub async fn add_handler(
    ...
    let resp = match outcome {
        InsertOutcome::Inserted { id } => AddResponse { outcome: "inserted", id },
        InsertOutcome::Skipped { id } => AddResponse { outcome: "skipped", id },
    };
    Ok(Json(resp))
}
